...
<div class="grid min-h-screen px-4 py-8 lg:grid-cols-[1fr_30rem] lg:px-0 lg:py-0">
    ...
    <main class="grid place-items-center">
        <form method="post" action="{{ route('login') }}" class="w-full max-w-md rounded-lg border border-line bg-white p-6 shadow-sm sm:p-8">
            @csrf
            <div class="mb-6">
            ...
            <label class="mb-4 block text-sm font-bold text-slate-700">Email
                <input name="email" type="email" value="{{ old('email') }}" required autofocus class="mt-1 w-full rounded-md">
            </label>
            ...
            @endif
            <a href="{{ route('register') }}" class="mt-4 block text-center text-sm font-bold text-ocean hover:underline">Daftar akun user</a>
        </form>
    </main>
</div>
@endsection
